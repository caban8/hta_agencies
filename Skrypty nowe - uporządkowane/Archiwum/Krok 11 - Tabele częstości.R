...
Analizy_częstości <-  Baza_analizy1 %>% mutate(Kat1_Korzyść_kliniczna2 = fct_recode(Kat1_Korzyść_kliniczna, 
  ...
  select(Kraj, Rekomendacja, liczba_agencji,
...
Analizy_wszystkie <- Analizy_częstości %>% 
  select(-liczba_agencji)



  Kategorie1 <- filtr_kategorie(Analizy_wszystkie)
  Kategorie2 <- filtr_kategorie(Analizy_wszystkie, kategoria = 2)
  ...
  nested_full3 <-   nested_full2 %>% 
    mutate(
      ...
      freq_all2 = map2(freq_all, name, kryterium_order, "Kraj"),
...
# Zapisywanie -------------------------------------------------------------
  ...
  for (i in 1:nrow(full_tabele)) {
    ...
    save_as_docx(full_tabele$flextabela[[i]], path = str_c("./Tabele częstości/Wszystkie leki/", full_tabele$Nazwa_tabeli[[i]], ".docx"))
  ...
  negatywne_all <- nested_full3 %>% 
    filter(Rekomendacja == "Negatywna")   %>% 
    select(freq_all2)
  
  negatywne_all$freq_all2 %>% 
      reduce(left_join, by = "Kraj") %>% 
    write_excel_csv("./Tabele częstości/Wszystkie leki/Wszystkie leki - negatywne.xlsx")
  
  
  pozytywne_all <- nested_full3 %>% 
    filter(Rekomendacja == "Pozytywna")   %>% 
    select(freq_all2)
  
  pozytywne_all$freq_all2 %>% 
      reduce(left_join, by = "Kraj") %>% 
    write_excel_csv("./Tabele częstości/Wszystkie leki/Wszystkie leki - pozytywne.xlsx")
...
# Zapisywanie -------------------------------------------------------------
  ...
  negatywne_reduced$freq_all2 %>% 
    reduce(left_join, by = "Kraj") %>% 
    write_excel_csv("./Tabele częstości/Zawężona próba/Zawężona próba - negatywne.xlsx")
  ...
  pozytywne_reduced$freq_all2 %>% 
    reduce(left_join, by = "Kraj") %>% 
    write_excel_csv("./Tabele częstości/Zawężona próba/Zawężona próba - pozytywne.xlsx")
