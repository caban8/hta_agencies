...
kappa_fun <- function(x, y) {
  ...
  # Dodaj gwiazdkę, gdy przedział ufności nie zawiera 0
  kappa <- if (all(result[-1] < 0)) {
    paste0(kappa, "*")
  } else if (all(result[-1] > 0)) {
    paste0(kappa, "*")
  } else  {kappa}
    
  
  # Doaj liczbę obserwacji
  output <- str_c(kappa, " [", lower, ", ", upper, "]", " \n(n = ", n, ")")
...
pivot_kappa <- df_kappa2 %>% 
  mutate(
    ...
    Kraj = plyr::mapvalues(Kraj, code_kraje$Agencies_old, code_kraje$Agencies_short) %>% 
      factor(levels = code_kraje$Agencies_short)
  ) %>% 
  arrange(Kraj) %>% 
