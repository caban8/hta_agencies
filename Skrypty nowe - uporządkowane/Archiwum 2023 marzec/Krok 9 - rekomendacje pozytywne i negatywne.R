...
# Krok 9 - Pozytywne i negatywne rekomendacje -----------------------------
...
funkcja_ggplot2 <- function(x, kategoria, grouping) {
  ...
  x$Freq %>% 
    ...
    ggplot(aes({{grouping}}, n, fill = .data[[kategoria]], alpha = Rekomendacja)) +
...
czestosci_kraje <- map(lista_kategorie, funkcja_czestosci2, x = Baza_analizy1, grouping = Kraj)
...
wykresy_kraje2
pliki_kraje <- str_c("Krok - 9 Rozkład pozytywnych i negatywnych rekomendacji ze względu na ", 
                     nazwy_kategorie, "(Kraje).png")

walk2(pliki_kraje, wykresy_kraje2, ggsave, device = "png", path = "./Wykresy/Wykresy 9",
      width = 12, height = 8, dpi = 300)




czestosci_obszar <- map(lista_kategorie, funkcja_czestosci2, x = Baza_analizy1, grouping = Obszar_terapeutyczny)
...
pliki_obszar <- str_c("Krok - 9 Rozkład pozytywnych i negatywnych rekomendacji ze względu na ", 
                     nazwy_kategorie, "(Obszary terapeutyczne).png")

walk2(pliki_obszar, wykresy_obszar2, ggsave, device = "png", path = "./Wykresy/Wykresy 9",
      width = 12, height = 8, dpi = 300)
