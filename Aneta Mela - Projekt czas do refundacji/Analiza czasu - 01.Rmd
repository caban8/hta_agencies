...
## Data analysis (for the article)

All analyses were conducted in R (version 4.3.2)
using Rstudio (2023.12.1, Build 402).
Data was prepared and visualized using the tidyverse package.

A mixed-effects model was applied to cater to the unbalanced data. Drug category, therapeutic area and country were used as fixed effects, while drug indication was used as the random effect with random intercepts. 
Number of days until recommendation was transformed logarithmically to allow for a more normal distribution of the residuals.
This was further improved by removing 19 outliers, using a standardized value of above |3| as the exclusion criterion. 
The model was obtained using the lme4 package. The pairwise comparisons between pairs of countries were calculated using the emmeans package.
As the model suffered from heteroscedasticity, the confidence intervals for mean differences were obtained using bootstrapping BCa with a 95% confidence level.




- Logarithmic transformation using natural logarithm was applied to improve the distribution of errors and to allow for a linear relationship
- Outliers were removed to achieve full normality of the errors
- Bootstrapping was used to provide robust standard errors given the heteroscedasticity of the data 
...
combined_posthoc[, -1][upper.tri(combined_posthoc[, -1], T)] <- ""
...
# Isolated estimated marginal means
