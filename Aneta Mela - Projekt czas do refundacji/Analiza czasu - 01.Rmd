...
df_main <- read_excel("d:/SPSS and statistics/Work/Zlecenia/Aneta Mela/Baza filtr na dates 2024-03-04.xlsx") 

df_main <- df_main %>% 
  select(-c(...1, ...2))

# Import additional observations updated at a later date
df_more <- read_excel("Dane - aktualizacja obserwacji 23.05.2024 .xlsx") %>% 
  select(-...1)

# Delete incorrectly parsed columns
df_more <- df_more %>% 
  select(-c(Data_rejestracji, Data_rekomendacji))
...
duplicated_id %>% 
  mutate(n = n - 1) %>% 
  group_by(Agencja_kraj) %>% 
  summarise(n = sum(n)) %>% 
  arrange(Agencja_kraj)



# Na przykładzie pojedynczego wskazania
df %>% 
  filter(ID == 1983)

df %>% 
  filter(ID == 1983) %>% 
  pull(Wskazanie_rejestracyjne) %>% 
  unique() 
```
...
df %>% 
  filter(ID == 1983) %>% 
  select(dni_do_rekomendacji, Kraj, Rekomendacja) %>% 
  arrange(Kraj, dni_do_rekomendacji)
...
## Remove "duplicated" recommendation per given indication
...
df %>% 
  drop_na(dni_do_rekomendacji, Drug_category, Therapeutic_lumped, Rekomendacja)
...
df <- df %>% 
  select(
    ID, Year, Nazwa_substancji_czynnej,
...
df %>%
  ...
  select(-Nazwa_substancji_czynnej) %>% 
  map(table)
```
...
## Change Virusology category to infectious diseases

Jednak nie zmieniamy. Zostawiamy wirusologię.

```{r}

# df <- df %>% 
#   mutate(Obszar_terapeutyczny = plyr::mapvalues(
...
df %>% 
  xlsx::write.xlsx(str_c("Time analysis data - 1933 observations (", Sys.Date(), ").xlsx"))
...
##  Number of drugs and indications authorized between 2014 and 2019 and covered in the analysis


```{r}

# Obtain columns with frequencies of the number of active substances, indications, and recommendations
drugs_freqs <- df %>% 
  select(Year, ID, Nazwa_substancji_czynnej) %>% 
  mutate(Year = as.character(Year)) %>% 
  nest(
    `Active substances` = Nazwa_substancji_czynnej, 
    Indications = ID
    ) %>% 
  mutate(
    Recommendations = map_dbl(Indications, nrow),
    across(-c(Year, Recommendations), ~map(., unique)),
    across(-c(Year, Recommendations), ~map_dbl(., nrow))
    ) %>% 
  bind_rows(
    summarise(
      .,
...
# Export drug freqs to a word table
drugs_freqs %>% 
  flextable() %>% 
  autofit() %>% 
  save_as_docx(path = "Tabele/Number of drugs and indications authorized between 2014 and 2019 and covered in the analysis.docx")
...
### Median time by country and year
...
stats_area <- df %>% 
  ...
  arrange(desc(median)) %>% 
   select(-c(variable,  mad,  se, ci)) %>% 
   Cabix2::round_df(q1, q3, iqr, mean, sd, digits = 1) %>% 
  select(Obszar_terapeutyczny, n, min, max, mean, sd, q1, median, q3, iqr) %>% 
  set_names(c("Therapeutic Area", "N", "Min", "Max","M",  "SD", "Q1", "Me", "Q3", "IQR"))
...
stats_area %>% 
  mutate(
    `areas year days` = if_else(Me > 365, "More than one year", "Less than one year")
  ) %>% 
  freq_table(`areas year days`) %>% 
  html_table()
```
...
stats_area %>%  
  ...
  font(fontname = "Times New Roman", part = "footer") %>% 
   save_as_docx(path = str_c("Tabele/Dni do rekomendacji a obszar.docx"))
...
### Summary stats by Country and Therapeutic aera


```{r}

stats_country_area <- df %>% 
  ...
  arrange(Kraj, desc(median )) %>% 
   select(-c(variable,  mad,  se, ci)) %>% 
   Cabix2::round_df(q1, q3, iqr, mean, sd, digits = 1) %>% 
    select(Kraj, Obszar_terapeutyczny, n, min, max, mean, sd, q1, median, q3, iqr) %>% 
    set_names(c("Country", "Therapeutic Area", "N", "Min", "Max","M",  "SD", "Q1", "Me", "Q3", "IQR"))
...
stats_country_area %>%  
  ...
  font(fontname = "Times New Roman", part = "footer") %>% 
   save_as_docx(path = str_c("Tabele/Dni do rekomendacji a kraj i obszar.docx"))
...
### Summary stats by Therapeutic area and Country 
...
stats_area_country <- df %>% 
   group_by(Obszar_terapeutyczny, Kraj, ) %>% 
   get_summary_stats(dni_do_rekomendacji) %>% 
   mutate(mean = round(mean, 1)) %>% 
  ungroup() %>% 
  arrange(Obszar_terapeutyczny, desc(median )) %>% 
   select(-c(variable,  mad,  se, ci)) %>% 
   Cabix2::round_df(q1, q3, iqr, mean, sd, digits = 1) %>% 
    select(Obszar_terapeutyczny, Kraj, n, min, max, mean, sd, q1, median, q3, iqr) %>% 
    set_names(c("Therapeutic Area", "Country", "N", "Min", "Max","M",  "SD", "Q1", "Me", "Q3", "IQR"))
...
stats_area_country %>%  
  ...
  font(fontname = "Times New Roman", part = "footer") %>% 
   save_as_docx(path = str_c("Tabele/Dni do rekomendacji a obszar i kraj.docx"))
...
```{r}
# Obtain medians, Q1, Q3, per country and year
stats <- df %>% 
...
# Extract a separate df with sample size for labelling
stats_n <- stats %>% 
  select(Kraj, Year, n) %>% 
  mutate(median = 1100)

stats_n <- stats %>% 
  select(Kraj, Year, n, q3) %>% 
  mutate(median = q3 + 110)
...
# Write iterated function for booted CI bca (back-transform version)
#boot_bca <- function(index, boot.out) {
#  
#  # Obtain bca ci
#  bca <- boot.ci(boot.out, type = "bca", index = index)
...
#    comparison = names(bca$t0), 
#    emmean = exp(bca$t0),
#    ci.lower = exp(bca$bca[4]),
#    ci.upper = exp(bca$bca[5])
#    )
#  
#  return(result)
#  
...
#### Problem z back-transform logarytmizacją

Problem polega na tym, że różnica obliczona po logarytmizacji, wrzucona następnie do funkcji wykładniczej, nie pokazuje prawdziwej różnicy, tylko stosunek (ratio) na skali prawdziwej

Yes, if the dependent variable of the model was log-transformed, the differences in estimated marginal means (EMMs) or contrasts from the emmeans package will indeed represent differences in the log-transformed scale. When you back-transform these differences from the log scale, they will represent ratios on the original scale.
...
### Emmeans without boostrap


```{r}
emmeans(fit_filtered, pairwise ~ Kraj, data = df_filtered, type = "response")
```
...
emm_means %>% 
 ...
 # geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL)) +
  geom_point(color = fill_color, size = 2.5) +
  theme_minimal() +
  theme(
  ...
  labs(x = "", y = "Mean Number of Days to Recommendation") 
