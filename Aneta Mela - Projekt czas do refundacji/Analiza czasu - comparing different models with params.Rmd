...
params:
  ...
  random_effects: 
    ...
    "(1|Year)", 
    "(1|Kraj)",
    "(1|Obszar_terapeutyczny)",
    # "(1 + ID | Kraj)",
    "(1|ID) + (1|Kraj)",
    "(1|ID) + (1|Year)",
    "(1|ID) + (1|Drug_category)",
    "(1|ID) + (1|Rekomendacja)",
    "(1|ID) + (1|Obszar_terapeutyczny)",
    "(1|Rekomendacja) + (1|Kraj)",
    "(1|Drug_category) + (1|Kraj)",
    "(1|Kraj) + (1|Kraj:Drug_category)",
    "(1|Obszar_terapeutyczny) + (1|Kraj)",
    "(1|Obszar_terapeutyczny) + (1|Kraj:Obszar_terapeutyczny)",
    # "(1|ID) + (1|ID:Kraj)",
    "(1|Year) + (1|Kraj)",
    "(1|Kraj) + (1|Kraj:Year)",
    "(1|Kraj) + (1|Year:Kraj)",
  ...
  weighted_var:
    ...
    value: Kraj
    input: select
    choices: [Kraj, Obszar_terapeutyczny, Therapeutic_lumped, Rekomendacja, Drug_category]
  weighted_model:
...
combinations <- length(control_simple)


# Create all formula combinations always including country as one of the fixed effects
control_simple2 <- map(1:combinations, combn, x = control_simple, simplify = F) %>% 
  unlist(recursive = F) %>% 
  map(sort) %>% 
...
control_int <- control_simple2 %>% 
  map(
    combn,
    m = 2,
    FUN = function(x) {str_c(x, collapse = ":")}
  ) %>% 
  {map2(control_simple2, ., c)}



if (params$Interactions) {
  combined_control <- c(control_simple2, control_int)
} else {combined_control <- control_simple2}





combined_control2 <- combined_control %>% 
  map(str_c, collapse = " + ") %>% 
  unlist() 
...
model_formulas <- expand_grid(dv, combined_control2, random_effects) %>% 
  mutate(
    mod_formula = future_pmap_chr(
      list(combined_control2, random_effects), 
      paste, 
      sep = " + "
      ),
...
models_df_unnested <-  models_df %>% 
  ...
  mutate(formula = fct_reorder(formula, AIC))
```
...
# Set random effects
random_weighted <- str_replace_all(params$random_effects, pattern = "\\(|\\)", "")
random_weighted <- str_c("~", random_weighted) %>% as.formula()
...
weighted_pred <- combined_control2
