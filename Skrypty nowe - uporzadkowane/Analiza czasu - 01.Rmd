...
Let's try implementing weights
...
weighted_mods <- expand.grid(
  DV = c("log_days", "dni_do_rekomendacji"),
  data = dataframes,
  method = c("ML", "REML"),
  weight_var = c("Kraj", "Therapeutic_lumped", "Rekomendacja")
) %>% 
  mutate(
    across(where(is.factor), ~as.character(.)),
    min_obs = rep(c(1, 2), nrow(.) / 2),
    mod_details = str_c(
      "DV = ", DV, 
      "; weights = ", weight_var, 
...
Jeśli nie uda się z weigthed version, to pozostaje obliczenie robust errors
...
library(emmeans)
emm <- emmeans(mixed_fit, pairwise ~ Kraj)
