...
```{r}
k <- sample(1:10, size = nrow(df), replace = T)

mod <- lmer(model_formulas$mod_formula[[1]], data = df[k != 1,])
predict(mod, newdata = df[k == 1,])


preds <- predict(models_df_unnested$model[[1]], newdata = df[sample(1:nrow(df), 100), ])


mean((df$dni_do_rekomendacji[1:100] - preds) ^ 2)



cross_pred <- function(formula, data) {
  
  # Create index for 10 cross-validations
  k <- sample(1:10, size = nrow(data), replace = T)
  
  # Run for loop on all cross validations
  mse <- vector(mode = "double")
  for (i in 1:10) {
    
    # Fit a model
    mod <- lmer(formula = formula, data = data[k != i, ])
    
    browser()
    
    # Obtain predictions
    preds <- predict(mod, newdata = data[k == i, ])
    
    
    
    # Obtain mse
    mse[i] <- mean(preds - data$dni_do_rekomendacji[k == i])
...
cross_pred(model_formulas$mod_formula[[1]], data = df)
